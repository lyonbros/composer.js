...
var ev=new CustomEvent(type,options.args);el.dispatchEvent(ev);};})();var find_parent=function(selector,element,stop){if(!element)return false;if(element==stop)return false;if(match(element,selector))return element;var par=element.parentNode;return find_parent(selector,par);};var frame=function(cb){global.requestAnimationFrame(cb);};var xdom={diff:function(from,to,options){return[from,to];},patch:function(root,diff,options){options||(options={});if(!root||!diff[1])return;var ignore_elements=options.ignore_elements||[];var ignore_children=options.ignore_children||[];return morphdom(root,diff[1],{onBeforeElUpdated:function(from,to){if(options.reset_inputs)return;var tag=from.tagName.toLowerCase();var from_type=from.getAttribute('type');var to_tag=to.tagName.toLowerCase();var to_type=to.getAttribute('type');if(to_tag=='input'&&to_type=='file'){if(tag=='input'&&from_type=='file'){var attrs=to.attributes;for(var i=0,n=attrs.length;i<n;i++){var key=attrs.item(i).name;if(key=='value')continue;from.setAttribute(key,to.getAttribute(key));}
return false;}
return;}
switch(tag)
