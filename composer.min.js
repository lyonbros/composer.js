 ...
 */
(function(){"use strict";if(!this.Composer){var Composer={version:'1.3.0',exp0rt:function(obj){Object.keys(obj).forEach(function(key){Composer[key]=obj[key];});}};this.Composer=Composer;}
...
this.add(model,options);return model;},clear:function(options){options||(options={});var num_rec=this._models.length;if(num_rec==0)return;this.remove(this._models,options);this._models=[];if(this._models.length!=num_rec){this.fire_event('clear',options,options);}},reset:function(data,options){options||(options={});if(!options.append&&!options.upsert)this.clear(options);if(options.upsert){var data_id_map={}
var id_key=(new this.model()).id_key;data.forEach(function(item){var id=item[id_key];data_id_map[id]=true;});var missing=[];this.each(function(m){var mid=m.id();if(!data_id_map[mid])missing.push(mid);});var remove_list=missing.map(function(mid){return this.get(mid);}.bind(this));this.remove(remove_list);this.upsert(data,options);}else{this.add(data,options);}
