...
return obj;};var create=function(base,mixinsfn){if('create'in Object){var prototype=Object.create(base.prototype);}else{base.$initializing=true;var prototype=new base();delete base.$initializing;}
var cls=function Omni(){if(cls.$initializing)return this;process_mixins(this,mixinsfn&&mixinsfn());copy(this);this.$state={parents:{},fn:[]};return this.initialize?this.initialize.apply(this,arguments):this;};cls.$constructor=prototype.$constructor=cls;cls.prototype=prototype;cls.prototype.$parent=base;return cls;};var extend_parent=function(to,from,k){return function(){if(!this.$state.parents[k])this.$state.parents[k]=[];this.$state.parents[k].push(from);this.$state.fn.push(k);var val=to.apply(this,arguments);this.$state.fn.pop();this.$state.parents[k].pop();return val;};};const process_mixins=function(obj,mixins){if(!Array.isArray(mixins)||mixins.length==0)return;const is_obj=function(o){return typeof(o)=='object'&&!Array.isArray(o);};const newobj={};const do_mixin=function(mixin){Object.keys(mixin).forEach(function(k){const val=mixin[k];if(is_obj(val)){newobj[k]=Object.assign({},newobj[k]||{},val);}else{newobj[k]=val;}});};mixins.forEach(function(mixin){do_mixin(mixin.prototype);});Object.keys(newobj).forEach(function(k){const val=newobj[k];if(typeof(obj[k])=='undefined'){obj[k]=val;}else if(is_obj(obj[k])&&is_obj(val)){obj[k]=Object.assign(val,obj[k]);}});};var do_extend=function(to_prototype,from_prototype){return merge(to_prototype,from_prototype,{transform:function(into,from,k){if(typeof into[k]!='function'||into[k].prototype.$parent||typeof from[k]!='function'||from[k].prototype.$parent)return false;from[k]=extend_parent(from[k],into[k],k);from[k].$parent=into[k];}});};var Base=function(){};Base.extend=function(obj){var base=this;const mixinsfn=obj._mixins?obj._mixins:false;delete obj._mixins;var cls=create(base,mixinsfn);do_extend(cls.prototype,obj);cls.extend=Base.extend;cls.prototype.$get_parent=function(){var k=this.$state.fn[this.$state.fn.length-1];if(!k)return false;var parents=this.$state.parents[k];var parent=parents[parents.length-1];return parent||false;};cls.prototype.parent=function(){var fn=this.$get_parent();if(fn)return fn.apply(this,arguments);throw'Class.js: Bad parent method: '+this.$state.fn[this.$state.fn.length-1];};return cls;};function Class(obj){return Base.extend(obj);};Class.extend=Class;Composer.exp0rt({Class:Class});}).apply((typeof exports!='undefined')?exports:this);(function(){"use strict";var Composer=this.Composer;var make_lookup_name=function(event_name,bind_name){return event_name+'@'+bind_name;};var Event=Composer.Class({_handlers:{},_handler_names:{},bind:function(event_name,fn,bind_name){if(Composer.array.is(event_name)){event_name.forEach(function(ev){this.bind(ev,fn,bind_name);}.bind(this));return this;}
