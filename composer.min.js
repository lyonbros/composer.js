 ...
 */
(function(){"use strict";if(!this.Composer){var Composer={version:'1.3.7',exp0rt:function(obj){Object.keys(obj).forEach(function(key){Composer[key]=obj[key];});}};this.Composer=Composer;}
...
var cls=function Omni(){if(cls.$initializing)return this;copy(this);this.$state={parents:{},fn:[]};return this.initialize?this.initialize.apply(this,arguments):this;};cls.$constructor=prototype.$constructor=cls;cls.prototype=prototype;cls.prototype.$parent=base;return cls;};var extend_parent=function(to,from,k){return function(){if(!this.$state.parents[k])this.$state.parents[k]=[];this.$state.parents[k].push(from);this.$state.fn.push(k);var val=to.apply(this,arguments);this.$state.fn.pop();this.$state.parents[k].pop();return val;};};const process_mixins=function(obj,mixins){if(!Array.isArray(mixins)||mixins.length==0)return obj;const is_obj=function(o){return typeof(o)=='object'&&!Array.isArray(o);};const newobj={};const do_mixin=function(mixin){Object.keys(mixin).forEach(function(k){const val=mixin[k];if(is_obj(val)){newobj[k]=Object.assign({},newobj[k]||{},val);}else{newobj[k]=val;}});};mixins.forEach(function(mixin){do_mixin(mixin.prototype);});do_mixin(obj);return newobj;};var do_extend=function(to_prototype,from_prototype){return merge(to_prototype,from_prototype,{transform:function(into,from,k){if(typeof into[k]!='function'||into[k].prototype.$parent||typeof from[k]!='function'||from[k].prototype.$parent)return false;from[k]=extend_parent(from[k],into[k],k);from[k].$parent=into[k];}});};var Base=function(){};Base.extend=function(obj){var base=this;var cls=create(base);const mixins=obj._mixins?obj._mixins():[];delete obj._mixins;obj=process_mixins(obj,mixins);do_extend(cls.prototype,obj);cls.extend=Base.extend;cls.prototype.$get_parent=function(){var k=this.$state.fn[this.$state.fn.length-1];if(!k)return false;var parents=this.$state.parents[k];var parent=parents[parents.length-1];return parent||false;};cls.prototype.parent=function(){var fn=this.$get_parent();if(fn)return fn.apply(this,arguments);throw'Class.js: Bad parent method: '+this.$state.fn[this.$state.fn.length-1];};return cls;};function Class(obj){return Base.extend(obj);};Class.extend=Class;Composer.exp0rt({Class:Class});}).apply((typeof exports!='undefined')?exports:this);(function(){"use strict";var Composer=this.Composer;var make_lookup_name=function(event_name,bind_name){return event_name+'@'+bind_name;};var Event=Composer.Class({_handlers:{},_handler_names:{},bind:function(event_name,fn,bind_name){if(Composer.array.is(event_name)){event_name.forEach(function(ev){this.bind(ev,fn,bind_name);}.bind(this));return this;}
...
this.add(model,options);return model;},clear:function(options){options||(options={});var num_rec=this._models.length;if(num_rec==0)return;this.remove(this._models,options);this._models=[];if(this._models.length!=num_rec){this.fire_event('clear',options,options);}},reset:function(data,options){options||(options={});if(!options.append&&!options.upsert)this.clear(options);if(options.upsert){if(!options.append){var data_id_map={}
var id_key=(new this.model()).id_key;data.forEach(function(item){if(item instanceof Composer.Model){var id=item.id();}else{var id=item[id_key];}
data_id_map[id]=true;});var missing=[];this.each(function(m){var mid=m.id();if(!data_id_map[mid])missing.push(mid);});var remove_list=missing.map(function(mid){return this.get(mid);}.bind(this));this.remove(remove_list);}
this.upsert(data,options);}else{this.add(data,options);}
...
if(!options.skip_release)this._subcontrollers[name].release();delete this._subcontrollers[name];},trigger_subs:function(_){var args=Array.prototype.slice.call(arguments,0);Object.keys(this._subcontrollers).forEach(function(name){var con=this.sub(name);if(con)con.trigger.apply(con,args);}.bind(this));},track_subcontroller:function(){return this.sub.apply(this,arguments);},get_subcontroller:function(name){return this.sub.apply(this,arguments);},remove_subcontroller:function(){return this.remove.apply(this,arguments);},_ensure_el:function(){if(typeof(this.el)=='string'){this.el=Composer.find(document,this.el);}
