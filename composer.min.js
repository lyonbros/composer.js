...
obj=obj[path];}
return obj;}};var promisify=function(poptions){poptions||(poptions={});var convert=function(type,asyncs){Object.keys(asyncs).forEach(function(key){var spec=asyncs[key];var options_idx=spec.options_idx||0;var names=spec.names||['success','error'];var _old=Composer[type].prototype[key];Composer[type].prototype[key]=function(){var args=Array.prototype.slice.call(arguments,0);if(args.length<options_idx){var _tmp=new Array(options_idx);args.forEach(function(item,i){_tmp[i]=item;});args=_tmp;}
...
var id=this.id(true);id=id?'/'+id:'';var url=base_url?'/'+base_url.replace(/^\/+/,'').replace(/\/+$/,'')+id:id;return url;}});this.Composer.exp0rt({Model:Model});}).apply((typeof exports!='undefined')?exports:this);(function(){"use strict";var global=this;var Collection=Composer.Base.extend({__composer_type:'collection',model:Composer.Model,_models:[],_id_idx:{},_cid_idx:{},sortfn:null,url:'/mycollection',priority:1,initialize:function(models,params,options){params||(params={});for(var x in params){this[x]=params[x];}
